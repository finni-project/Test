...
export default function SearchBox(){
    ...
    const [searchRes, setSearchRes] = useState(searchResultData);
    ...
    return(
        <div>
            <SearchBar>
                <input type="text" placeholder="용돈 분류나 내용을 검색해보세요" value={input} onChange={handleInputChange}/>
            ...
            (<ResultBoxWrapper>
                <h2>관련 내용</h2>
                {searchResultData.map((itm)=>{
                    return(
                        <List key={itm.id}>
                            <ResultBox>
                                <p>{itm.contents}</p>
                                <span>{itm.date.replace(/-/g, ".")}</span>
                            </ResultBox>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
